...
export default function Reader({ bookRecord, theme, onThemeChange, onClose }: ReaderProps) {
  ...
  function installImageViewer(rendition = engineRef.current?.rendition) {
    ...
    for (const contents of rendition.getContents?.() ?? []) {
      ...
      contentDocument.addEventListener('click', (event) => {
        ...
        setSelection(null)
        setImagePreview({ src: source, alt: image.getAttribute('alt') || image.getAttribute('aria-label') || 'EPUB 插图', zoom: 1, offsetX: 0, offsetY: 0 })
      }, true)
    }
  }
  ...
  useEffect(() => {
    ...
    async function initialize() {
      try {
        ...
        if (!disposed) { setError(friendlyReaderError('epub', reason)); setLoading(false) }
      }
    }
    initialize()
    return () => {
  ...
  return <main className="reader-shell">
    <header className="reader-bar"><button className="icon-button" onClick={onClose} aria-label="返回书架"><ArrowLeft size={19} /></button><div className="reader-title"><strong>{bookRecord.title}</strong><ReadingSessionStatus bookId={bookRecord.id} progress={progress} /></div><div className="reader-controls"><button className="icon-button" onClick={() => setTocOpen(true)} aria-label="目录"><BookOpenText size={19} /></button><AnnotationPanel annotations={annotationStore.annotations} onAddBookmark={addBookmark} onJump={jumpAnnotation} onRemove={removeAnnotation} /><button className="icon-button" onClick={() => { setSearchOpen(true); setTimeout(() => searchInputRef.current?.focus(), 0) }} aria-label="搜索 EPUB"><Search size={18} /></button><div className="setting-group" title="字号"><button onClick={() => updateSetting('fontSize', Math.max(12, settings.fontSize - 1))}><Minus size={14} /></button><span>{settings.fontSize}</span><button onClick={() => updateSetting('fontSize', Math.min(36, settings.fontSize + 1))}><Plus size={14} /></button></div><ReadingSettings settings={settings} onChange={updateSetting} /><button className="icon-button" onClick={onThemeChange} aria-label="切换主题">{theme === 'dark' ? <Sun size={18} /> : <Moon size={18} />}</button></div></header>
    <section ref={stageRef} className="reading-stage" onWheel={onWheel}>{settings.readingMode === 'paginated' && <button className="page-hit left" onClick={() => engineRef.current?.rendition.prev()} aria-label="上一页"><ChevronLeft /></button>}<div className="epub-wrap" style={{ maxWidth: useDoublePage ? settings.contentWidth * 2 + 120 : settings.contentWidth + 120 }}><div ref={viewportRef} className="epub-viewport" /></div>{settings.readingMode === 'paginated' && <button className="page-hit right" onClick={() => engineRef.current?.rendition.next()} aria-label="下一页"><ChevronRight /></button>}{loading && <div className="reader-message">正在打开…</div>}{error && <div className="reader-message error"><strong>无法打开 EPUB</strong><span>{error}</span></div>}{searchOpen && <form className="pdf-search-bar" onSubmit={(event) => { event.preventDefault(); searchBook() }}><Search size={16} /><input ref={searchInputRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索 EPUB 正文" /><span>{searching ? '搜索中…' : results.length ? `${resultIndex + 1} / ${results.length}` : query ? '无结果' : ''}</span><button type="button" onClick={() => moveResult(-1)} disabled={!results.length}><ChevronLeft size={16} /></button><button type="button" onClick={() => moveResult(1)} disabled={!results.length}><ChevronRight size={16} /></button><button type="button" onClick={() => setSearchOpen(false)}><X size={16} /></button></form>}</section>
    <div className="progress-track"><i style={{ width: `${progress * 100}%` }} /></div>
    {tocOpen && <div className="drawer-backdrop" onMouseDown={() => setTocOpen(false)}><aside className="toc-drawer" onMouseDown={(event) => event.stopPropagation()}><div className="drawer-head"><strong>目录</strong><button className="icon-button" onClick={() => setTocOpen(false)}><X size={18} /></button></div>{toc.length ? <ol className="toc-list">{renderToc(toc)}</ol> : <p className="muted">此书没有可用目录</p>}</aside></div>}
    {imagePreview && <div className="image-preview-backdrop" role="presentation" onMouseDown={() => setImagePreview(null)}><div className="image-preview" role="dialog" aria-modal="true" aria-label="查看插图" onMouseDown={(event) => event.stopPropagation()}><div className="image-preview-toolbar"><button className="icon-button" onClick={() => updateImageZoom(-.25)} aria-label="缩小插图" disabled={imagePreview.zoom <= .5}><Minus size={18} /></button><button className="image-preview-zoom" onClick={() => updateImageZoom('reset')} aria-label="适应窗口">{Math.round(imagePreview.zoom * 100)}%</button><button className="icon-button" onClick={() => updateImageZoom(.25)} aria-label="放大插图" disabled={imagePreview.zoom >= 4}><Plus size={18} /></button><button className="icon-button image-preview-close" onClick={() => setImagePreview(null)} aria-label="关闭插图"><X size={20} /></button></div><div className="image-preview-stage" onWheel={onImageWheel} onPointerDown={onImagePointerDown} onPointerMove={onImagePointerMove} onPointerUp={onImagePointerEnd} onPointerCancel={onImagePointerEnd}><img src={imagePreview.src} alt={imagePreview.alt} style={{ transform: `translate(${imagePreview.offsetX}px, ${imagePreview.offsetY}px) scale(${imagePreview.zoom})` }} /></div></div></div>}
